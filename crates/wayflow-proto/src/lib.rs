...
mod tests {
    ...
    fn rt<T: serde::Serialize + serde::de::DeserializeOwned + PartialEq + core::fmt::Debug>(val: &T) {
    ...
    fn modifiers_all_true() {
        rt(&Modifiers { shift: true, ctrl: true, alt: true, meta: true });
    }
    ...
    #[test]
    fn s2c_hello() {
        rt(&S2C::Hello(HelloS2C { version: 1, screens: alloc::vec![] }));
    }
    ...
    fn s2c_enter_screen() {
        ...
        rt(&S2C::EnterScreen { x: u16::MAX, y: u16::MAX });
    }
    ...
    fn s2c_mouse_move_abs() {
        ...
        rt(&S2C::MouseMoveAbs { x: u16::MAX, y: u16::MAX });
    }

    #[test]
    fn s2c_mouse_button() {
        rt(&S2C::MouseButton { button: MouseButton::Left, pressed: true });
        rt(&S2C::MouseButton { button: MouseButton::Right, pressed: false });
        rt(&S2C::MouseButton { button: MouseButton::Other(7), pressed: true });
    }
    ...
    fn s2c_scroll() {
        ...
        rt(&S2C::Scroll { dx: i16::MIN, dy: i16::MAX });
    }
    ...
    fn s2c_key_event() {
        ...
        rt(&S2C::KeyEvent {
            keycode: u32::MAX,
            pressed: true,
            modifiers: Modifiers { shift: true, ctrl: true, alt: true, meta: true },
        });
    }

    #[test]
    fn s2c_clipboard_data() {
        rt(&S2C::ClipboardData(ClipboardContent::Text("paste me".into())));
    }
    ...
    fn c2s_clipboard_data() {
        rt(&C2S::ClipboardData(ClipboardContent::Text("from client".into())));
    }
