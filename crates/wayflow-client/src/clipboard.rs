use tokio::sync::mpsc;
use wayflow_proto::{ClipboardContent, C2S};
...
mod platform {
    ...
    use wayflow_proto::{ClipboardContent, ClipboardImage, C2S};

    const POLL_INTERVAL: Duration = Duration::from_millis(750);
    const MAX_CLIPBOARD_BYTES: usize = 3 * 1024 * 1024;

    pub fn start(outbound: mpsc::Sender<C2S>) -> mpsc::Sender<ClipboardContent> {
            ...
            loop {
                ...
                if let Some(content) = snapshot(&mut clipboard) {
                    if content_size(&content) > MAX_CLIPBOARD_BYTES {
                        ...
                        if outbound.blocking_send(C2S::ClipboardData(content)).is_err() {
                            break;
                        }
                    }
                }
...
mod platform {
    ...
    pub fn start(_outbound: mpsc::Sender<C2S>) -> mpsc::Sender<ClipboardContent> {
